...
- Interface for authorising and revoking third party tools.

## 0.4.19 - 2026-06-29

### Fixed

- Bug where an organisation's provider admin users were being shown to superadmins, which was then causing the form to fail validation because the full details for provider admins weren't being populated in the form.

## 0.4.18 - 2026-05-19

### Changes

- Refactor settings to use the best practice of importing settings from django.conf.
- Refactor settings to move constant data (e.g., codelists, timezone lists) from settings.
- Change database connection to configure from a single connection string URL, simplifying the configuration of the application.
- Removed environment variables for the paths to codelist and licence JSON files, simplifying the configuration of the application.

### Fixed

- Small bug where "Superadmin" is listed as a possible user role in the list of users for an organisation.

### Added

- Unit test of some of the OIDC code.
